...
$card_query = "SELECT * favorite WHERE user_id='$email'";
$card_run_query = mysqli_query($conn, $card_query);
while($fav= mysqli_fetch_assoc($card_run_query)){
$ff = $fav['user_id'];
echo $ff;
    $r_card_query = "SELECT * recipe WHERE id='$recipe_id'";
    $r_card_run_query = mysqli_query($conn, $r_card_query);
    while($view= mysqli_fetch_assoc($r_card_run_query)){
